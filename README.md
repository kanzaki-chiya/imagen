...
- **历史记录与资产管理**
  ...
  - 删除先进入回收站（保留 30 天可恢复），彻底删除会同时清理本地图片与缩略图文件。
  - 详情弹窗完整回溯元数据与参考图，支持复制图片到剪贴板、在系统文件管理器中定位原图或另存为导出。
  - 2K/4K 档位向兼容服务商请求原生大尺寸，超出其输出上限时自动本地放大并在详情中标注。
- **创意预设库**
  - 内置丰富预设模版，支持自定义保存常用参数组合与提示词方案，一键快速复用。
- **沉浸式现代桌面体验**
  - 采用无边框设计与自定义系统标题栏，深度适配 1080p / 1440p 桌面显示。
  - 自适应响应式布局：窗口缩小时侧边栏自动收缩为紧凑图标，参数面板切换为侧滑抽屉。
  - 支持剪贴板直达工作流：Ctrl+V 粘贴剪贴板图片为参考图，生成结果一键复制到剪贴板。
  - 启动时静默检查更新，发现新版本可一键下载安装并重启（基于 Tauri Updater 签名包）。
